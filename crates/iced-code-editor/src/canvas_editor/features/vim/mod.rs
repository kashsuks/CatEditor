...
impl VimState {
    ...
    fn parse_operator(&mut self, operator: VimOperator) -> Option<VimAction> {
        ...
        if let Some(pending) = self.pending_operator {
            if pending == operator {
                let count = self.pending_operator_count.saturating_mul(self.take_count());
                self.clear_pending();
    ...
    fn push_count_digit(&mut self, key: char) {
        ...
        self.count = Some(self.count.unwrap_or_default().saturating_mul(10).saturating_add(digit));
    }
